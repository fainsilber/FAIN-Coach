...
**Read first:** [docs/PRD.md](docs/PRD.md) (requirements) and [docs/dev-plan.md](docs/dev-plan.md) (v3.4 — authoritative for schema, sprints, and decisions; supersedes the PRD wherever they conflict).
...
- **Garmin's login is the only part that needs Python.** Measured 2026-07-31: `connectapi.garmin.com` (data) *and* `diauth.garmin.com` (token refresh) both answer plain TLS, so the Worker does them with `fetch`. Only the login needs curl_cffi/CAPTCHA/MFA handling, which is why it stays on the user's machine and only tokens travel. Don't "simplify" by moving login server-side — that also funnels every user through one IP, and 429s were measured on a single residential one.
